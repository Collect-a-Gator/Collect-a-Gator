    ...
    useEffect(() => {
        if (isFirstRender.current) {
            ...
            const fetchData = async () => {
              const response = await fetch("http://${NEXT_PUBLIC_API_URL}/api/entries", {
                  method: "POST",
                  body: JSON.stringify({
                      title: title,
                      date: date,
              ...
              const getUserData = async () => {
                try {
                    const response = await fetch("http://${NEXT_PUBLIC_API_URL}/api/users/" + user?.id, {
                        method: "GET",
                        headers: {
                            "Content-Type": "application/json; charset=UTF-8"
                        }
            ...
            const adjustCounters = async () => {
             ...
             //   console.log("Cafe Counter: ", counters.cafe);
              ...
              await fetch(`http://${NEXT_PUBLIC_API_URL}/api/users/` + user?.id, {
                method: "PUT",
                body: JSON.stringify({
                counters: {
                  uf: counters.uf,
